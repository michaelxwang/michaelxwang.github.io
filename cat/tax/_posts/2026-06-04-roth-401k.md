...
As before, we find the break even point by evaluating the take home pay at withdrawal time:
...
Using the same set of parameters except where noted, we calculated 5 different tax rate scenarios and 4 time ranges under each. For each horizon:

- The first number is the break‑even ordinary marginal rate (guess‑and‑verify).  
- The second number is the break‑even effective rate from Equation (5).
